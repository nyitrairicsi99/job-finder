...
class MyServer(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path=="/" or self.path=="/favicon.ico":
            ...
            self.end_headers()
            self.wfile.write(bytes("<html><head><title>Profession.hu állások</title></head>", "utf-8"))
            ...
            for row in file:
                ...
                if len(row)>1: #nem elválasztó
                    if row[1]=="0":
                        self.wfile.write(bytes("<p><a href='"+row[5]+"' target='_blank'><strong>"+row[2]+"</strong></a><br />"+row[3]+"<br />"+row[4]+"<br /><i style='cursor: pointer;' onclick=deleterow("+(row[0])+")>Törlés</i></p>", "utf-8"))
            file.close()
